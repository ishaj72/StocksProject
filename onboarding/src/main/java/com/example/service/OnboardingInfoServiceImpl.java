...
public class OnboardingInfoServiceImpl implements IOnboardingInfoService {
    ...
    private static final Logger log = (Logger) LoggerFactory.getLogger(OnboardingInfoServiceImpl.class);

    @Autowired
    IOnboardingInfoDao onboardingInfoDao;

    @Override
    @Transactional
    public void saveOnboardingInfo(OnboardInfo wrapper) {
        if(ObjectUtils.isEmpty(wrapper)){
            log.error("OnboardInfo cannot be null or empty");
            throw new IllegalArgumentException("OnboardInfo cannot be null or empty");
        }
        
        log.info("Saving onboarding info for user: {}", wrapper.getUserId());
        onboardingInfoDao.save(wrapper);
        log.info("Successfully saved onboarding info");
    }
